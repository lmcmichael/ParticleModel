...
std_dev = 0.5*width_1std_100_polluted(t_start)*1000.0; %1 std dev in meters 
mu = 0;
...
for m = 1:ens_num
    ...
    downsamplingfactor = ceil((double(length(new_time))/double(length(width_1std_100_polluted))));
...
figure;
plot(t(t_start:72),width_1std_100_polluted(t_start:72),'LineWidth',5,'color',[0.0 0.0 0.0])
xlabel('Time since injection (hrs)')
ylabel('Plume Width (km)')
hold on
plot(t(t_start:71),ens_width((t_start+1):72),'LineWidth',5,'color','b')
for m=1:ens_num
    plot(t(t_start:71),part_width_avg((t_start+1):72,m),'LineWidth',0.0001,'color','magenta','LineStyle','-.');
end
plot(t(t_start:72),width_1std_100_polluted(t_start:72),'LineWidth',5,'color',[0.0 0.0 0.0])
plot(t(t_start:71),ens_width((t_start+1):72),'LineWidth',5,'color','b')
ylim([0 70]);
xlim([0 36]);
title('Polluted Ensemble Results (domain average, modified relaxation timescale)')
pbaspect([1.5 1 1]);
patch([15.5 15.5 25.0 25.0 15.5],[0 120 120 0 0], [0.7 0.7 0.7],'FaceAlpha',0.1);
legend('Polluted (LES)','PM Ensemble Avg.','PM Ensemble Members','Location','southeast')
exportgraphics(gcf,'/Users/mcmichael/Desktop/S12_figures/polluted_DA_fit.png')

%plot ensemble members for domain average
% figure;
% plot(t(t_start:45),width_1std_100_polluted(t_start:45),'LineWidth',5,'color',[0.0 0.0 0.0])
% xlabel('Time since injection (hrs)')
% ylabel('Plume Width (km)')
% hold on
% plot(t(t_start:44),ens_width_st1((t_start+1):45),'LineWidth',5,'color','b')
% for m=1:ens_num
%     plot(t(t_start:44),part_width_avg_st1((t_start+1):45,m),'LineWidth',0.0001,'color','magenta','LineStyle','-.');
% end
% ylim([0 70]);
% xlim([0 36]);
% plot(t(t_start:45),width_1std_100_polluted(t_start:45),'LineWidth',5,'color',[0.0 0.0 0.0])
% plot(t(t_start:44),ens_width_st1((t_start+1):45),'LineWidth',5,'color','b')
% title('Polluted Ensemble Results (in-plume average, modified relaxation timescale)')
% pbaspect([1.5 1 1]);
% patch([15.5 15.5 25.0 25.0 15.5],[0 120 120 0 0], [0.7 0.7 0.7],'FaceAlpha',0.1);
% legend('Polluted (LES)','PM Ensemble Avg.','PM Ensemble Members','Location','southeast')
% exportgraphics(gcf,'/Users/mcmichael/Desktop/S12_figures/polluted_IP_fit.png')

%plot absolute error between LES and PM
for n=2:length(t)
    err(n) = ens_width(n) - width_1std_100_polluted(n-1);
    %err_st1(n) = ens_width_st1(n) - width_1std_100_polluted(n-1);
    %err_st2(n) = ens_width_st2(n) - width_1std_100_polluted(n-1);
end

figure;
plot(t(t_start:71),err((t_start+1):72),'LineWidth',3,'color','b','LineStyle','-')
xlabel('Time since injection (hrs)')
ylabel('Particle Model Plume Width Error (km)')
hold on
%plot(t(t_start:44),err_st1((t_start+1):45),'LineWidth',3,'color','magenta','LineStyle','-')
%plot(t(t_start:71),err_st2(t_start+1:72),'LineWidth',1.5,'color','cyan','LineStyle','--')
ylim([-10 10]);
xlim([0 36]);
plot([min(t), max(t)], [0, 0], 'k--');
title('PM and LES comparison (Polluted)')
pbaspect([2 1 1]);
patch([15.5 15.5 25.0 25.0 15.5],[-50 120 120 -50 -50], [0.7 0.7 0.7],'FaceAlpha',0.1);
legend('domain average error','in-plume error','Location','southeast')
exportgraphics(gcf,'/Users/mcmichael/Desktop/S12_figures/polluted_error_fit.png')
