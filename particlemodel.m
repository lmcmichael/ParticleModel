...
std_dev_vel = sqrt(MEAN_U2(1));
mean_u_vel = MEAN_U(1);

part_vel_init = mean_u_vel + std_dev_vel * randn(part_num,1);

part_vel_init_st1 = mean_u_vel + std_dev_vel * randn(part_num,1);

part_vel_init_st2 = mean_u_vel + std_dev_vel * randn(part_num,1);
...
figure;
histogram(part_pos_0(:,1), 30); % Adjust the number of bins as needed
title('Normal Distribution of Particles');
xlabel('zonal position (km)');
ylabel('Frequency');

%plot zonal velocity of initial distribution
figure;
histogram(part_vel_0(:,1), 30); % Adjust the number of bins as needed
title('Normal Distribution of Particles');
