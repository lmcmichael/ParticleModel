...
std_dev = 0.5*width_1std_100_strongshear(t_start)*1000.0; %1 std dev in meters 
mu = 0;
...
for m = 1:ens_num
    ...
    downsamplingfactor = ceil((double(length(new_time))/double(length(width_1std_100_strongshear))));
...
figure;
plot(t(t_start:72),width_1std_100_strongshear(t_start:72),'LineWidth',3,'color',[0.0 0.0 0.0])
...
ylim([0 70]);
xlim([0 36]);
title('Strong Shear Ensemble Results (domain average)')
pbaspect([2 1 1]);
patch([15.5 15.5 25.0 25.0 15.5],[0 120 120 0 0], [0.7 0.7 0.7],'FaceAlpha',0.1);
legend('Strong Shear (LES)','PM Ensemble Avg.','PM Ensemble Members','Location','southeast')
...
ylim([0 70]);
xlim([0 36]);
title('Strong Shear Ensemble Results (in-plume average)')
pbaspect([2 1 1]);
patch([15.5 15.5 25.0 25.0 15.5],[0 120 120 0 0], [0.7 0.7 0.7],'FaceAlpha',0.1);
legend('Strong Shear (LES)','PM Ensemble Avg.','PM Ensemble Members','Location','southeast')
...
ylim([0 70]);
xlim([0 36]);
title('Strong Shear Ensemble Results (plume-edge average)')
pbaspect([2 1 1]);
patch([15.5 15.5 25.0 25.0 15.5],[0 120 120 0 0], [0.7 0.7 0.7],'FaceAlpha',0.1);
legend('Strong Shear (LES)','PM Ensemble Avg.','PM Ensemble Members','Location','southeast')
...
for n=2:length(t)
    err(n) = ens_width(n) - width_1std_100_strongshear(n-1);
    err_st1(n) = ens_width_st1(n) - width_1std_100_strongshear(n-1);
    err_st2(n) = ens_width_st2(n) - width_1std_100_strongshear(n-1);
end
...
title('PM and LES comparison (Strong Shear)')
pbaspect([2 1 1]);
